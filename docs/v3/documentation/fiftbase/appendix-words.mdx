...
This appendix lists core Fift words with their stack effects. It is being generated from the original PDF; below is a curated subset with pointers to the full reference. A compact A–Z index of letter‑starting keywords is also included for quick lookup.
...
The list below includes letter‑starting Fift keywords (ordinary words) extracted from the highlighter grammar; symbol‑prefixed words (e.g., `"`, `#`, `$`, `+`, `-`, etc.) and low‑level operators are covered in the sections that follow.
