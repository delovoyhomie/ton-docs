...
There is a [TVM exit code](/v3/documentation/tvm/exit-codes) for a transaction that had a compute phase. If it is not 0 or 1, then there was an error.
Also, TVM [compute phase may be skipped](/v3/documentation/tvm/overview#when-the-compute-phase-is-skipped) for reasons like lack of funds or state.
...
- Fail, [exit code](/v3/documentation/tvm/exit-codes), `aborted: true`
